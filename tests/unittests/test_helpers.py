...
def test_unix_ms_to_date():
    test_cases = [{"case": 1629181750735, "expected": "2021-08-17T06:29:10"},
                  {"case": -145222249876, "expected": "1965-05-26T04:29:10"},
                  {"case": 1441761672555, "expected": "2015-09-09T01:21:12"}]
...
def test_datetime_to_unix_ms():
    test_cases = [{"case": datetime.datetime(2021, 8, 16, 23, 29, 10, 735719), "expected": 1629156550735},
                  {"case": datetime.datetime(1965, 5, 25, 21, 29, 10, 123456), "expected": -145247449876},
                  {"case": datetime.datetime(2015, 9, 8, 18, 21, 12, 555555), "expected": 1441736472555}]
...
def test_create_csid_params():
    test_cases = [{"case": (i for i in range(5)), "expected": {"csids": "0,1,2,3,4"}},
...
def test_get_next_page_key():
    test_cases = [
        {
            "case": "https://www.example.com/api/v1/some/path?some=query_string", 
            "expected": {"some": "query_string"}
        },
        {
            "case": "/api/v1/some/path?qs=abc123", 
            "expected": {"qs": "abc123"}
        },
        {
            "case": None, 
...
def test_date_to_rfc3339() :
    test_cases = [{"case": "2021-08-01 00:00:00+00:00", "expected": "2021-08-01T00:00:00Z"}]

    for case in test_cases:
        assert case["expected"] == helpers.date_to_rfc3339(case["case"])
